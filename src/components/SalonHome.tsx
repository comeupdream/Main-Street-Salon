...
/**
 * The full salon landing page. `showIntro` plays the theatrical reveal overlay
 * (used on /demo); the public "/" renders without it.
 */
export async function SalonHome({ showIntro = false }: { showIntro?: boolean }) {
  const categories = await getServicesByCategory();
  const hours = hoursForDisplay();

  return (
    <>
      {showIntro && <RevealIntro />}
      <SiteHeader />
