import { SALON, salonNowHM, salonTodayISO } from "./salon-config";
...
export function computeAvailableSlots(
  ...
  const hours = SALON.hours[weekdayOf(dateISO)];
  if (!hours) return []; // closed that day

  const openMin = timeToMinutes(hours.open);
  const closeMin = timeToMinutes(hours.close);
  const step = SALON.slotIntervalMinutes;

  // Earliest allowed start if the date is today.
  let earliest = openMin;
  if (dateISO === salonTodayISO()) {
    const nowMin = timeToMinutes(salonNowHM()) + SALON.minLeadMinutes;
